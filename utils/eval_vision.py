...
def save_pil(pil_img, path: Path):
...
def write_examples(task_name, examples, out_dir: Path):
    ex_dir = out_dir / "examples" / task_name
    ex_dir.mkdir(parents=True, exist_ok=True)
    manifest = []
    for i, ex in enumerate(examples):
        img_name = f"{i+1:02d}_idx{ex['idx']}.png"
        txt_name = f"{i+1:02d}_idx{ex['idx']}.json"
        img_path = ex_dir / img_name
        if ex.get("image") is not None:
            save_pil(ex["image"], img_path)
        payload = {
            "idx": ex["idx"],
            "query": ex["query"],
            "gold": ex.get("gold"),
        ...
        (ex_dir / txt_name).write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
        manifest.append({"image": img_name, "meta": txt_name})
    (ex_dir / "manifest.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")


def run_ocr(ds_path, out_jsonl, api, model, n=100, examples_k=5, out_dir=None):
    ...
    with open(out_jsonl, "w", encoding="utf-8") as f:
        for i in range(total):
            ...
            is_err = (pred is None) or (isinstance(resp, str) and resp.startswith("[ERROR]"))
            rec = {
                "idx": i,
                "query": query,
            ...
            if len(examples) < examples_k:
                try:
                    # Download image_url for archive
                    r = requests.get(row["image_url"], timeout=30)
                    if r.status_code == 200:
                        from PIL import Image
                        image = Image.open(io.BytesIO(r.content))
                        examples.append({**rec, "image": image})
                except Exception:
                    pass
    ...
    if out_dir is not None and examples:
        write_examples("ocr", examples, out_dir)
    return {"total": total, "passed": passed, "accuracy": passed / max(1, total)}


def run_classification(ds_path, out_jsonl, api, model, n=100, examples_k=5, out_dir=None):
    ds = load_from_disk(ds_path)
    ...
    with open(out_jsonl, "w", encoding="utf-8") as f:
        for i in range(total):
            ...
            if len(examples) < examples_k:
                examples.append({**rec, "image": row["img"]})

            if (i + 1) % 10 == 0:
                print(f"CLS {i+1}/{total} pass={passed}", flush=True)

    if out_dir is not None and examples:
        write_examples("classification", examples, out_dir)
    return {"total": total, "passed": passed, "accuracy": passed / max(1, total)}


def run_detection(ds_path, out_jsonl, api, model, n=100, examples_k=5, out_dir=None):
    ds = load_from_disk(ds_path)
    ...
    with open(out_jsonl, "w", encoding="utf-8") as f:
        for i in range(total):
            ...
            if len(examples) < examples_k:
                examples.append({**rec, "image": row["image"]})

            if (i + 1) % 10 == 0:
                print(f"DET {i+1}/{total} pass={passed}", flush=True)

    if out_dir is not None and examples:
        write_examples("detection", examples, out_dir)
    return {"total": total, "passed": passed, "accuracy": passed / max(1, total)}
...
def main():
    ...
    ap.add_argument("--examples", type=int, default=5, help="number of per-benchmark image+response examples to save")
    ap.add_argument("--out-dir", default="/Users/yenhaohuang/Desktop/model-tester/ministral-3-14b-vision/results")
    ap.add_argument("--ocr-ds", default="/Users/yenhaohuang/Desktop/datasets/vision-dataset/PokemonCards_train_300")
    ap.add_argument("--cls-ds", default="/Users/yenhaohuang/Desktop/datasets/vision-dataset/cifar10_classification")
    ap.add_argument("--det-ds", default="/Users/yenhaohuang/Desktop/datasets/vision-dataset/object_detection")
    ...
    t0 = time.time()
    ocr = run_ocr(args.ocr_ds, str(out_dir / f"ocr_pokemoncards_{args.num}.jsonl"), args.api, args.model, n=args.num, examples_k=args.examples, out_dir=out_dir) if "ocr" in wanted else None
    cls = run_classification(args.cls_ds, str(out_dir / f"classification_cifar10_{args.num}.jsonl"), args.api, args.model, n=args.num, examples_k=args.examples, out_dir=out_dir) if "classification" in wanted else None
    det = run_detection(args.det_ds, str(out_dir / f"detection_cppe5_{args.num}.jsonl"), args.api, args.model, n=args.num, examples_k=args.examples, out_dir=out_dir) if "detection" in wanted else None
