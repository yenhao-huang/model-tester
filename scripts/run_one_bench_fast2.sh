#!/usr/bin/env bash
set -euo pipefail
KEY="$1"; LAUNCH="$2"; PORT="$3"; MODEL_NAME="$4"; BENCH="$5"
DATASET_ROOT="$HOME/Desktop/datasets/fast-textgen-evalset2"
BASE_DIR="$HOME/Desktop/model-tester"
TS="$(date +%Y%m%d_%H%M%S)"
RUN_DIR="$BASE_DIR/$KEY/runs/${TS}_fasttextgenevalset2_${BENCH}"
mkdir -p "$RUN_DIR"
...
for i in $(seq 1 240); do
  if curl -s "http://127.0.0.1:${PORT}/v1/chat/completions" -H 'Content-Type: application/json' \
    -d '{"model":"'"$MODEL_NAME"'","messages":[{"role":"user","content":"say READY"}],"temperature":0.0,"max_tokens":8}' | grep -q '"choices"'; then
    READY=1; break
  fi
...
echo "server ready; eval $BENCH start" | tee -a "$RUN_DIR/run.log"
